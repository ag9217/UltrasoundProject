...
transducer.element_width = 1.0;       % width of each element [grid points]
...
transducer.focus_distance = x - 40e-3;              % focus distance [m]
...
% Keeping random number generation to default so randn produces the dsame
% random numbers
rng('default'); 
...
sound_speed_map(1:round((x_pos - 40e-3)/dx),:,:) = 1480;
density_map(1:round((x_pos - 40e-3)/dx),:,:) = 1000;
% reapplying randomness to newly defined layer
sound_speed_map(1:round((x_pos - 40e-3)/dx),:,:) = sound_speed_map(1:round((x_pos - 40e-3)/dx),:,:) .* background_map(1:round((x_pos - 40e-3)/dx),:,:);
