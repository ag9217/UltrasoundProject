...
% defining skin layer (1mm = Nx/128 voxels ish)
sound_speed_map(Nx/2:Nx/2+Nx/128,:,:) = 1624;
density_map(Nx/2:Nx/2+Nx/128,:,:) = 1050;
% reapplying randomness to newly defined layer
sound_speed_map(Nx/2:Nx/2+Nx/128,:,:) = sound_speed_map(Nx/2:Nx/2+Nx/128,:,:) .* background_map(Nx/2:Nx/2+Nx/128,:,:);

% defining subcut fat (24mm = 24 * Nx/128 voxels)
sound_speed_map(Nx/2+Nx/128:Nx/2+25*Nx/128,:,:) = 1450;
density_map(Nx/2+Nx/128:Nx/2+25*Nx/128,:,:) = 900;
% reapplying randomness to newly defined layer
sound_speed_map(Nx/2+Nx/128:Nx/2+25*Nx/128,:,:) = sound_speed_map(Nx/2+Nx/128:Nx/2+25*Nx/128,:,:) .* background_map(Nx/2+Nx/128:Nx/2+25*Nx/128,:,:);

% defining muscle (12mm)
sound_speed_map(Nx/2+25*Nx/128:Nx/2+(37*Nx/128),:,:) = 1580;
density_map(Nx/2+25*Nx/128:Nx/2+(37*Nx/128),:,:) = 1100;
% reapplying randomness to newly defined layer
sound_speed_map(Nx/2+25*Nx/128:Nx/2+(37*Nx/128),:,:) = sound_speed_map(Nx/2+25*Nx/128:Nx/2+(37*Nx/128),:,:) .* background_map(Nx/2+25*Nx/128:Nx/2+(37*Nx/128),:,:);

% define a sphere for a highly scattering region
radius = 8e-3;
x_pos = 32e-3;
y_pos = dy * Ny/2;
scattering_region1 = makeBall(Nx, Ny, Nz, Nx - radius/(dx) - 10, round(Ny/2), round(Nz/2), round(radius / dx));
